	...
	public void processPacket(Packet packet) {
		...
		if (packet instanceof Presence) {
			...
			if (presence.getType().equals(Presence.Type.subscribe)) {// 好友申请
				...
				if (!XmppUtil.getRosterAll(QApp.getXmppConnection().getRoster())
						.contains(fromJid)) {
				...
				if (XmppUtil.getRosterTo(QApp.getXmppConnection().getRoster())
						.contains(fromJid)) {
					Logger.e(TAG, fromJid + ":to");
					XmppUtil.sendAgreeAddFriend(QApp.getXmppConnection(), fromJid);
				...
				Log.e(TAG, "拒绝添加好友");
				 ...
				 SessionDao.getInstance().deleteSessionByYou(fromJid);
				   ChatMsgDao.getInstance().deleteYouMsg(fromJid);
					QApp.getInstance().sendBroadcast(new Intent(
